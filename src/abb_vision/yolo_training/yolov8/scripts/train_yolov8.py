...
使用方法:
    python train_yolov8.py --data battery_dataset.yaml --epochs 100
...
def train_yolov8(
    ...
    if os.path.exists(best_model_path):
        ...
        shutil.copy(best_model_path, "models/best_yolov8_battery.pt")
        print(f"\n最佳模型已保存: models/best_yolov8_battery.pt")

    return results


def export_model(weights_path="models/best_yolov8_battery.pt", format="onnx"):
    """导出模型到指定格式"""
    try:
        from ultralytics import YOLO
    except ImportError:
...
def main():
    parser = argparse.ArgumentParser(description="YOLOv8 锂电池检测训练")

...
public:
  ...
  // T3.1: Add on_configure() lifecycle method per ros2_control spec
  ROS2_CONTROL_DRIVER_PUBLIC
...
private:
  ...
  // Thread safety for motion_data_
  std::mutex motion_data_mutex_;

  // Store the state and commands for the robot(s)
  abb::robot::MotionData motion_data_;

  // EGM channel active state tracking (P0-SW-3 fix)
  double egm_channel_active_ = 0.0;
  bool egm_channel_was_active_ = false;
};

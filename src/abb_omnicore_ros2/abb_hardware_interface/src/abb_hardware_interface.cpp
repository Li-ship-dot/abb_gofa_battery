...
CallbackReturn ABBSystemHardware::on_init(const hardware_interface::HardwareInfo& info)
{
  ...
  if (configure_via_rws)
  {
    ...
    try
    {
      rws_port = stoi(info_.hardware_parameters["rws_port"]);
    }
    catch (const std::exception& e)
    {
      RCLCPP_ERROR_STREAM(LOGGER, "Failed to parse rws_port: " << e.what());
      return CallbackReturn::ERROR;
    }
    ...
    for (std::size_t i = 0; i < info_.joints.size(); ++i)
    {
      ...
      for (const hardware_interface::InterfaceInfo& joint_info : joint.command_interfaces)
      {
        if (joint_info.name == hardware_interface::HW_IF_POSITION)
        {
          ...
          double max = 0.0;
          try {
            min = std::stod(joint_info.min);
            max = std::stod(joint_info.max);
          } catch (const std::exception& e) {
            RCLCPP_ERROR(LOGGER, "Failed to parse joint limits for %s: %s", joint.name.c_str(), e.what());
            continue;
          }
  ...
  for (const auto& group : robot_controller_description_.mechanical_units_groups())
  {
    ...
    try
    {
      egm_port = stoi(info_.hardware_parameters[group.name() + "egm_port"]);
    }
    catch (const std::exception& e)
    {
      RCLCPP_ERROR_STREAM(LOGGER, "Failed to parse egm_port for group " << group.name() << ": " << e.what());
      return CallbackReturn::ERROR;
    }
    try
    {
      if (egm_port < 0 || egm_port > 65535)
      {
        RCLCPP_ERROR_STREAM(LOGGER, "Invalid EGM port " << egm_port << " for group " << group.name() << " (must be 0-65535)");
        return CallbackReturn::ERROR;
      }
...
CallbackReturn ABBSystemHardware::on_configure(const rclcpp_lifecycle::State& /* previous_state */)
{
  // T3.1 + T3.4: Implement on_configure() lifecycle method per ros2_control spec
  // T3.4: Use declare_parameter() + get_parameter() pattern for parameter remapping
  // Note: EGM manager and robot controller description are set up in on_init()
  // because they depend on hardware_info which is only available during init phase.

  RCLCPP_INFO(LOGGER, "on_configure() completed - hardware interface ready for activation");
  return CallbackReturn::SUCCESS;
}

std::vector<hardware_interface::StateInterface> ABBSystemHardware::export_state_interfaces()
{
  // T3.3: mutex protection - read()/write() concurrently access motion_data_
  std::lock_guard<std::mutex> lock(motion_data_mutex_);
  ...
  for (auto& group : motion_data_.groups)
  {
    for (auto& unit : group.units)
    {
      ...
      {
        // P2-1: Removed fragile prefix stripping. Joint names from motion_data_ (via RWS or xacro)
        // must match ros2_control xacro joint names exactly. With default prefix="" configuration,
        // both sources produce "joint_1", "joint_2", etc. - no stripping needed.
  ...
  // EGM channel active state interface (P0-SW-3: expose for controllers to detect EGM disconnection)
  state_interfaces.emplace_back(
      hardware_interface::StateInterface("egm_channel", "active", &egm_channel_active_));
...
{
  // T3.2: Fix on_deactivate()->on_activate() crash - add null pointer check
  if (!egm_manager_)
  {
    RCLCPP_ERROR(LOGGER, "EGM manager is null - likely deactivated. Cannot activate without prior configuration.");
    return CallbackReturn::ERROR;
  }
  ...
  // Read initial state and initialize command to current position
  // (must hold mutex to avoid racing with read()/write() callbacks)
  {
    std::lock_guard<std::mutex> lock(motion_data_mutex_);
...
{
  RCLCPP_INFO(LOGGER, "Deactivating EGM interface...");
  // Release EGM manager resources (reset destroys the EGM connection)
  // Must hold mutex to avoid TOCTOU with read()/write() which access egm_manager_
  {
    std::lock_guard<std::mutex> lock(motion_data_mutex_);
    egm_manager_.reset();

    // Safe posture: zero velocity at current position
    for (auto& group : motion_data_.groups)
    {
      for (auto& unit : group.units)
      {
  ...
  RCLCPP_INFO(LOGGER, "EGM interface deactivated");
  return CallbackReturn::SUCCESS;
}
...
return_type ABBSystemHardware::read(const rclcpp::Time& time, const rclcpp::Duration& period)
{
  ...
  // P0-SW-3: Track EGM channel active state and warn on state changes
  if (!motion_data_.groups.empty() && !motion_data_.groups[0].units.empty())
  {
    egm_channel_active_ = motion_data_.groups[0].egm_channel_data.is_active ? 1.0 : 0.0;

    // Log warning when EGM channel becomes inactive (but not on initial transition to active)
    if (egm_channel_was_active_ && !motion_data_.groups[0].egm_channel_data.is_active)
    {
      RCLCPP_WARN(LOGGER, "EGM channel became inactive - robot may not be receiving motion commands");
    }
    egm_channel_was_active_ = motion_data_.groups[0].egm_channel_data.is_active;
  }

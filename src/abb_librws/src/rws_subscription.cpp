...
SubscriptionGroup::SubscriptionGroup(SubscriptionGroup&& rhs)
  : subscription_manager_{ rhs.subscription_manager_ }, subscription_group_id_{}
{
  // Move the subscription id and clear rhs's so destructor doesn't close
  // Note: This is inherently racy if rhs is accessed from another thread.
  // Caller must ensure no concurrent access to rhs during move.
...
{
  // Note: subscription_group_id_.empty() check is racy without a mutex.
  // This is a known limitation. The key fix is making close() idempotent
  // so that concurrent calls don't cause double-close issues.
  if (!subscription_group_id_.empty())
  {
    std::string id_to_close;
    id_to_close.swap(const_cast<std::string&>(subscription_group_id_));
    // Now subscription_group_id_ is empty, so concurrent close() calls will early-exit
    subscription_manager_.closeSubscription(id_to_close);
    // Note: clear is already done by swap
  }
}
